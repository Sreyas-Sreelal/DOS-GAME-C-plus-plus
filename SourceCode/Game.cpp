...
public:
  ...
  void
  controller(); // function for getting player inputs to move the game object
  void
  update(); // function that will update game screen on each events in the game
...
public:
  ...
  void
  Chart() // another display function that will help to print top 10 in chart
  {
  ...
  char*
  getpass() // accessor function to get password (used only in temp objects)
  {
    return password;
  }
...
game_data::controller()
{
  ...
  if (x == width || y == height || x == 0 ||
      y == 0) // if the game object hits the wall
    gameover = 1;
...
save()
{
  ...
  for (int i = 0; i < data_size; i++) {
    file.write(
      (char*)&s[i],
      sizeof(s[i])); // write the details of updated player class in all
  }
  file.close();
}
...
connect()
{
  ...
  while (!file.eof()) {
    ...
    file.read(
      (char*)&s[i],
      sizeof(
        s[i])); // initialising player class objects with registered players
    if (file.eof())
      break;
    i++;
  }
...
void
menu(player_data& temp)
{
  clrscr();
  int res /*to get response in the main menu*/,
    res2 /*response in search menu*/;
