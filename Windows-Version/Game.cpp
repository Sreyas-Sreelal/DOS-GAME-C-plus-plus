#include "stdafx.h"
#include<iostream>
#include<fstream> //for file handling (saving score name and password)
#include<conio.h>//for getch_,_kbhit textcolor and text background
#include<cstdlib>//for rand() functions
//#include<process>//for exit() function
#include<stdio.h>//for gets and other functions
#include<string>//for string functions strlen,strcmp etc
...
s[25]/*global object contains all details of registerd players*/, temp/*object holds temporary player data*/;


void game_data::controller()
{
	if (_kbhit())//gets key input
	{
		switch (_getch())
		{
...
void main()
{
	...
	if (logined)//if the player logged in
	{
		...
		while (!game.gameover)
		{
			game.controller();
			game.move();
			Sleep(150);//the looping speed is decreased using this so as to provide an excellent gameplay mechanism
		}
	}
	system("cls");
	cout << "\t\t!!!! GAME OVER !!!!!\n";
